...
namespace ArcFace.BLL
{
    public class AsfFaceControl
    {
        ...
        private int GetFaceFeature(byte[] imageData, out AsfStruct.ASF_FaceFeature faceFeature, out int faceNumber)
        {
            faceNumber = 0;
            ...
            faceNumber = detectedFaces.faceNum;
        ...
        public int GetFaces(Image img, out List<Bitmap> faceList)
        {
            faceList = new List<Bitmap>();
            byte[] imageData = ImageHelper.ImageToBytes(img, ImageFormat.Jpeg);
            Bitmap faceBitmap = ImageHelper.BytesToBitmap(imageData);
            List<FaceInfoModel> faces = new List<FaceInfoModel>();
            int res = GetFaceInfo(imageData, out faces);
            if (res != AsfConstants.MOK)
                return res;
            ...
            {
                Rectangle faceRect = face.faceRect.GetRectangle();
                Bitmap singleFace = new Bitmap(faceRect.Width, faceRect.Height);
                using (Graphics drawG = Graphics.FromImage(singleFace))
                {
                    drawG.DrawImage(img, 0, 0, faceRect, GraphicsUnit.Pixel);
                }
                faceList.Add(singleFace);
            }
        ...
        /// <summary>
        /// 根据原始图像，提取图像中的所有人脸以及人脸特征
        /// </summary>
        /// <param name="sourceImage"></param>
        /// <param name="faceData"></param>
        /// <returns></returns>
        public int GetFaceDatas(Bitmap sourceImage, ref ImageFaceDataModel faceData)
        {
            ImageDataModel image = ImageDataConverter.ConvertToImageData(sourceImage);
            AsfStruct.ASF_MultiFaceInfo detectedFaces = new AsfStruct.ASF_MultiFaceInfo();
            int res = AsfFunctions.ASFDetectFaces(hEngine, image.Width, image.Height, AsfConstants.AsfFacePixelFormat.ASVL_PAF_RGB24_B8G8R8, image.PImageData, out detectedFaces);
            if (res != AsfConstants.MOK)
                return res;
            ...
            for (int i = 0; i < detectedFaces.faceNum; ++i)
            {
                ...
                res = AsfFunctions.ASFFaceFeatureExtract(hEngine, image.Width, image.Height, AsfConstants.AsfFacePixelFormat.ASVL_PAF_RGB24_B8G8R8, image.PImageData, ref singleFaceInfo, out faceFeature);
                if (res != AsfConstants.MOK)
                    continue;
                Bitmap singleFaceImage = GetSingleFaceImage(sourceImage, singleFaceInfo);
                ...
                {
                    Face = singleFaceImage,
                    FaceFeature = featureData
                });
            }

            return AsfConstants.MOK;
        }
        /// <summary>
        /// 提取单人人脸特性值
        /// </summary>
        ...
        /// <returns></returns>
        public int ExtractFaceFeature(byte[] imageData, ref byte[] featureData)
        {
            AsfStruct.ASF_FaceFeature faceFeature = new AsfStruct.ASF_FaceFeature();
            int faceNumer = 0;
            int res = GetFaceFeature(imageData, out faceFeature, out faceNumer);
            if (res != AsfConstants.MOK)
                return res;
        ...
        public int ExtractFaceFeature(byte[] imageData, ref byte[] featureData, out int faceNumber)
        {
            faceNumber = 0;
            AsfStruct.ASF_FaceFeature faceFeature = new AsfStruct.ASF_FaceFeature();
            int res = GetFaceFeature(imageData, out faceFeature, out faceNumber);
            if (res != AsfConstants.MOK)
                return res;
            featureData = new byte[faceFeature.featureSize];
            Marshal.Copy(faceFeature.feature, featureData, 0, featureData.Length);
        ...
        /// <returns></returns>
        public FaceSampleModel FaceMatchingByFeature(byte[] feature1, float simileScore)
        {
            ...
            IntPtr f1 = CommonMethod.BytesToIntptr(feature1);
            faceFeature1.feature = Marshal.AllocCoTaskMem(feature1.Length);
            faceFeature1.featureSize = feature1.Length;
            CommonMethod.CopyMemory(faceFeature1.feature, f1, feature1.Length);
            ...
            while (modelsIEnumerator.MoveNext())
            {
                ...
                faceFeature2.feature = Marshal.AllocCoTaskMem(feature2.Length);
                faceFeature2.featureSize = feature2.Length;
                CommonMethod.CopyMemory(faceFeature2.feature, f2, feature2.Length);

                AsfFunctions.ASFFaceFeatureCompare(hEngine, ref faceFeature1, ref faceFeature2, out match_Result);
                if (match_Result >= simileScore)
                {
        ...
        public int FaceMatching(byte[] imageData1, byte[] imageData2, out float similscore)
        {
            ...
            try
            {
                ...
                int res = GetFaceFeature(imageData1, out faceFeature1, out faceNumber);
                if (res != AsfConstants.MOK)
                    return res;

                res = GetFaceFeature(imageData2, out faceFeature2, out faceNumber);
                if (res != AsfConstants.MOK)
                    return res;
        ...
        /// </summary>
        /// <param name="matchImg">被匹配的图像</param>
        /// <param name="similar">给定相似值</param>
        /// <returns>无匹配结果返回null</returns>
        public FaceSampleModel FaceMatching(byte[] matchImg, float similar)
        {
            FaceSampleDAL faceSampleDAL = new FaceSampleDAL();
            float match_Result = 0.0F;
            FaceSampleModel model = null;
